module xlsxImport {
    export function drawWorksheet(workbook: wijmo.xlsx.IWorkbook, sheetIndex: number, rootElement: HTMLElement,
        maxRows, maxColumns) {
    ...
    function importStyle(cssStyle: CSSStyleDeclaration, xlsxStyle: wijmo.xlsx.IWorkbookStyle) {
        if (!xlsxStyle) {
            return;
        }
        if (xlsxStyle.fill) {
