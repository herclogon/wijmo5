module wijmo.chart.research {
    "use strict";

    export class AggregateSeries extends Series {
        // backing fields
        ...
        private _groupAggregate: Aggregate = Aggregate.Avg;
        ...
        set autoInterval(value: boolean) {
            if (this._autoInterval !== value) {
                this._autoInterval = asBoolean(value, false);
                this._clearValues();
                this._invalidate();
            }
        }
        ...
        set autoGroupIntervals(value: string[]) {
            if (this._autoGroupIntervals !== value) {
                value = asArray(value, false);
                value.forEach((val: string) => assert(this._isValidInterval(asString(val, false)), "Invalid autoGroupIntervals"), this);
        ...
        set autoMaxGroupings(value: number) {
            if (this._autoMaxGroupings !== value) {
                this._autoMaxGroupings = asInt(value, false, true);
                this._clearValues();
                this._invalidate();
            }
        }
        ...
        set groupInterval(value: string) {
            if (this._groupInterval !== value) {
                if (value !== null && value !== "null") {
                    assert(this._isValidInterval(asString(value, true)), "Invalid groupInterval");
                }
                this._groupInterval = asString(value, true);
                this._clearValues();
                this._invalidate();
            }
        }
        ...
        get groupAggregate(): Aggregate {
            return this._groupAggregate;
        }
        set groupAggregate(value: Aggregate) {
            if (this._groupAggregate !== value) {
                this._groupAggregate = asEnum(value, Aggregate, false);
                this._clearValues();
                this._invalidate();
            }
        }
        ...
        private _rangeChangedHandler(sender: Axis): void {
        ...
        private _applyGroup(key: string, aggregate: Aggregate): void {
            var cv = new collections.CollectionView(this.collectionView.items),
                match: string[], gd: collections.PropertyGroupDescription,
            ...
            for (row = 0; row < this._allValues[key].length; row++) {
                this._allAxisLabels[key].push({
                    ...
                    text: Globalize.formatDate(this._allValues[key][row][this.bindingX], this._getFormatX(interval))
                });
            }
        }
        ...
        private _selectInterval(xmin: number, xmax: number): void {
            if (!this._autoInterval || !this._isGrouped || (!isFinite(xmin) || !isNumber(xmin)) || (!isFinite(xmax) || !isNumber(xmax))) {
                return;
            }
        ...
        private _updateAxes(subset: boolean): void {
            ...
            // update axisY limits
            if (isFinite(ymin) && isNumber(ymin) && ymin !== Number.MAX_VALUE) {
                ay.min = ymin;
            }
            if (isFinite(ymax) && isNumber(ymax) && ymax !== Number.MIN_VALUE) {
                ay.max = ymax;
            }
        ...
        private _getGroupDescription(bindingX: string, interval: string, subInterval: number = 1): collections.PropertyGroupDescription {
            var fn: Function = null;
            subInterval = subInterval || 1;
            switch (interval) {
                case "YYYY":
                    fn = (item: any, propName: string) => {
                        var year = asDate(item[propName]).getFullYear();
                        return new Date(year, 0, 1);
                    };
                    break;
                case "MM":
                    fn = (item: any, propName: string) => {
                        var d = asDate(item[propName]),
                            month = d.getMonth(),
                            year = d.getFullYear();
                ...
                case "ss":
                    ...
                    break;
                default:
                    assert(false, "Invalid groupInterval");
                    break;
            }

            return new collections.PropertyGroupDescription(bindingX, fn);
        }
        ...
        private _canGroup(): boolean {
            var retval = isString(this.binding) && isString(this.bindingX) && this.groupAggregate !== Aggregate.None && this.collectionView !== null && !isUndefined(this.collectionView);
        ...
        private _isValidInterval(interval: string): boolean {
            ...
            return ["ss", "mm", "hh", "DD", "WW", "MM", "YYYY"].indexOf(interval) >= 0 && isInt(subInterval);
        }
